...
double pause_time = 5, simulator_stop_time = 10.01;//修改流持续时间原先值：3.01
...
void ScheduleFlowInputs(){
	while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()){
		...
		clientHelper.SetAttribute("DataRate", StringValue("30Gbps"));
		...
		if(flow_input.stop_time>flow_input.start_time){
			if(flow_input.stop_time<simulator_stop_time-0.001){
				appCon.Stop(Seconds(flow_input.stop_time));
			}else{
...
void ScheduleAttackerInputs(){
	for(set<int>::iterator node=attack_nodes.begin();node!=attack_nodes.end();node++){
		...
		while(curr_start<simulator_stop_time-0.001){
			...
			RdmaClientHelper clientHelper(3,serverAddress[*node],serverAddress[attacker_dst],port,100,2000000,has_win?(global_t==1?maxBdp:pairBdp[n.Get(*node)][n.Get(attacker_dst)]):0, global_t==1?maxRtt:pairRtt[*node][attacker_dst]);
